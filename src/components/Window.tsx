...
const Window = ({ title, children, className = '' }: WindowProps) => {
  return (
    ...
    >
      <div className="bg-gradient-to-r from-window-border to-muted px-4 py-3 flex items-center gap-2 border-b border-window-border">
        <div className="flex gap-2">
          <div className="w-3 h-3 rounded-full bg-destructive"></div>
          <div className="w-3 h-3 rounded-full bg-[#FFB800]"></div>
          <div className="w-3 h-3 rounded-full bg-terminal-green"></div>
        </div>
        <span className="font-mono text-sm text-foreground ml-2">{title}</span>
      </div>
